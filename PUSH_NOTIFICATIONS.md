...
| **게스트 참가 확정** *(훈련 대관)* | 참가자 | 게스트 참가 확정 🏒 | `{경기장} ({시간}) 게스트 참가가 확정되었습니다. ({금액}원 차감)`<br>*(또는 게스트 참가 신청 완료)* | `/match/{id}` |
| **팀 매칭 확정** | 참가자 | 팀 매칭 확정 🤝 | `{경기장} ({시간}) 팀 매칭이 확정되었습니다.` | `/match/{id}` |
...
| **팀 매칭 취소** | 참가자 | 팀 매칭 취소 ↩️ | `팀 매칭 신청이 취소되었습니다.` | `/mypage` |
...
| 상황 (Trigger) | 수신자 (Recipient) | 제목 (Title) | 메시지 내용 (Body) | 이동 (Link) |
| :--- | :--- | :--- | :--- | :--- |
| **새 참가자 신청** *(게임 대관)* | 경기 생성자 (본인 제외) | 새 참가자 🏒 | `{이름}님이 {경기장} ({시간}) 경기에 신청했습니다. [(장비대여)]` | `/admin/matches` |
| **새 게스트 신청** *(훈련 대관)* | 경기 생성자 (본인 제외) | 새 게스트 🏒 | `{이름}님이 {경기장} ({시간}) 훈련에 게스트로 신청했습니다. [(장비대여)]` | `/admin/matches` |
| **상대팀 매칭 신청** *(팀 매치)* | 경기 생성자 (본인 제외) | 상대팀 매칭 신청 🤝 | `{이름}님이 {경기장} ({시간}) 팀 매칭에 상대팀으로 신청했습니다.` | `/admin/matches` |
| **새 대기자 신청** *(게임 대관)* | 경기 생성자 (본인 제외) | 새 대기자 ⏳ | `{이름}님이 {경기장} ({시간}) 경기에 대기 신청했습니다. [(장비대여)]` | `/admin/matches` |
| **새 게스트 대기 신청** *(훈련 대관)* | 경기 생성자 (본인 제외) | 새 대기자 ⏳ | `{이름}님이 {경기장} ({시간}) 훈련에 게스트 대기 신청했습니다. [(장비대여)]` | `/admin/matches` |
...
## 4. 관리자 시스템/게시글 등 (어드민 / 사용자 대상)

| 상황 (Trigger) | 수신자 (Recipient) | 제목 (Title) | 메시지 내용 (Body) | 이동 (Link) |
| :--- | :--- | :--- | :--- | :--- |
| **링크장 승인 완료** | 링크장을 생성한 일반 관리자 | 링크장 승인 완료 🏟️ | `요청하신 '{링크장명}' 링크장의 승인이 완료되었습니다.` | `/admin/rinks` |
...
| **새 링크장 승인 요청** | 🔔 알림: OTHER | `{email}님이 새 링크장 '{링크장명}' 승인을 요청했습니다. (승인 대기 중)` |
| **경기 생성** | 🔔 알림: MATCH_CREATE | `[{팀명}] {관리자}님이 새 매치를 생성했습니다. (일시: {시간})` |
| **경기 삭제** | 🔔 알림: MATCH_DELETE | `[{팀명}] {관리자}님이 {경기장} ({시간}) 매치를 삭제했습니다.` |
| **경기 참가** *(게임 대관)* | 🔔 알림: MATCH_JOIN | `{이름}님이 {경기장} 경기에 참가 확정했습니다.` |
| **게스트 참가** *(훈련 대관)* | 🔔 알림: MATCH_JOIN | `{이름}님이 {경기장} 훈련에 게스트로 참가 확정했습니다.` |
| **경기 취소** | 🔔 알림: MATCH_CANCEL | `{email}님이 경기 참가를 취소했습니다. (환불: {금액}원)` |
| **충전 요청** | 🔔 알림: POINT_CHARGE_REQUEST | `{email}님이 {금액}원 충전을 요청했습니다.` |

## 6. 포인트 거래 내역 (`point_transactions.description`)

| 상황 | description |
| :--- | :--- |
| **경기 참가** *(게임 대관)* | `경기 참가` / `경기 참가 (장비 대여 포함)` |
| **게스트 참가** *(훈련 대관)* | `게스트 참가` / `게스트 참가 (장비 대여 포함)` |
| **경기 취소 환불** | `경기 취소 환불 ({퍼센트}%)` |
| **대기 승격** | `대기 승격 및 참가비 결제` / `대기 승격 및 참가비 결제 (장비 대여 포함)` |
| **관리자 취소 환불** | `관리자 취소 환불 (100%)` |
