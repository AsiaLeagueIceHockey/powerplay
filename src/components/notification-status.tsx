...
import { getSubscriptionStatus, getSubscriptionStatusByEndpoint } from "@/app/actions/push";
...
export function NotificationStatus() {
  ...
  const [currentDeviceRegistered, setCurrentDeviceRegistered] = useState<boolean | null>(null);
        ...
        if ("serviceWorker" in navigator && "PushManager" in window) {
          try {
            ...
            if (reg?.active) {
              ...
              if (sub) {
                const epStatus = await getSubscriptionStatusByEndpoint(sub.endpoint);
                setCurrentDeviceRegistered(epStatus.exists);
              } else {
  ...
  const thisDeviceOk = currentDeviceRegistered === true;
        ...
        <div className="flex-1 min-w-0">
          ...
          {hasSubscription ? (
            <div className="mt-1 space-y-1">
              ...
              {currentDeviceRegistered !== null && (
                <p className={`text-xs flex items-center gap-1 ${thisDeviceOk ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`}>
                  {thisDeviceOk ? "✅ 이 기기: 등록됨" : "⚠️ 이 기기: 미등록"}
                </p>
              )}
