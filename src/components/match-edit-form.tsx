...
export function MatchEditForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const feeRaw = ((formData.get("fee") as string) || "").replace(/,/g, "");
    const parsedFee = feeRaw ? parseInt(feeRaw, 10) : 0;
    // match_type is handled by radio input automatically in formData

    if (currentMatchType !== "team_match" && parsedFee > MAX_MATCH_ENTRY_POINTS) {
        ...
        <div className="relative">
          <input
            ...
            onChange={(e) => {
              ...
              const limited = value ? Math.min(Number(value), MAX_MATCH_ENTRY_POINTS) : 0;
              e.target.value = value ? limited.toLocaleString() : "";
            }}
