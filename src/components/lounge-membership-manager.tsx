...
export function LoungeMembershipManager({
  ...
  const formatAdminLabel = (admin: AdminOption) => {
    ...
    const roleLabel = admin.role === "superuser"
      ? (locale === "ko" ? "슈퍼유저" : "Superuser")
      : (locale === "ko" ? "관리자" : "Admin");
    const details = [secondary, roleLabel].filter(Boolean).join(" | ");
    ...
    <div className="space-y-5 rounded-3xl border border-zinc-700 bg-[radial-gradient(circle_at_top_left,_rgba(251,191,36,0.12),_transparent_28%),linear-gradient(180deg,#3f3f46_0%,#27272a_100%)] p-6 shadow-sm">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-400">
          {description || (locale === "ko" ? "관리자 또는 슈퍼유저 계정에 라운지 구독 기간을 수기 등록합니다." : "Manually assign lounge contract periods to admin or superuser accounts.")}
        </p>
      </div>
      ...
      >
        ...
        <label className="space-y-2 text-sm md:col-span-2">
          <span className="font-medium text-zinc-100">{locale === "ko" ? "대상 운영 계정" : "Target admin-level account"}</span>
          <select
            name="user_id"
            required
          ...
          >
            <option value="">{locale === "ko" ? "관리자 또는 슈퍼유저를 선택하세요" : "Select an admin or superuser"}</option>
          ...
          <p className="text-xs leading-5 text-zinc-400">
            {locale === "ko"
              ? "동명이인 구분을 위해 이름, 이메일, 전화번호와 권한이 함께 표시됩니다."
              : "Name, email, phone, and role are shown together for identification."}
          </p>
        </label>
