...
export function MatchApplication({
  ...
  const hasStarted = new Date(matchStartTime) <= new Date();
  ...
  if (hasStarted) {
    return (
      <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-6 text-center dark:border-zinc-800 dark:bg-zinc-900/60">
        <div className="flex items-center justify-center gap-2">
          <span className="font-bold text-zinc-700 dark:text-zinc-200 text-lg">
            {locale === "ko" ? "종료된 경기입니다" : "This match has ended"}
          </span>
        </div>
      </div>
    );
