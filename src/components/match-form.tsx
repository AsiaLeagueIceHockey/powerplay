...
export function MatchForm({ rinks, clubs = [] }: MatchFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const rawEntryPoints = ((formData.get("entry_points") as string) || "").replace(/,/g, "");
    const parsedEntryPoints = rawEntryPoints ? parseInt(rawEntryPoints, 10) : 0;

    if (parsedEntryPoints > MAX_MATCH_ENTRY_POINTS) {
      setError(t("admin.form.entryPointsLimitError"));
      setLoading(false);
      return;
    }
      ...
      {!isTeamMatch && (
        <>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  const limited = raw ? Math.min(Number(raw), MAX_MATCH_ENTRY_POINTS) : 0;
                  const formatted = raw ? limited.toLocaleString() : "";
            ...
            <p className="text-xs text-zinc-500 mt-1">
              {t("admin.form.entryPointsLimitHint")}
            </p>
          </div>
