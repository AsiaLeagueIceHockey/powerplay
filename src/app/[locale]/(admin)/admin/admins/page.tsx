...
export default async function AdminListPage({
    ...
    const activeTab = tab === "users" || tab === "chat" ? tab : "admins";

    // Parallel fetch based on active tab
    const [admins, allUsers, chatRooms] = await Promise.all([
        activeTab === "admins" ? getAdmins() : Promise.resolve([]),
        activeTab === "users" ? getAllUsers() : Promise.resolve([]),
        activeTab === "chat" ? getSuperuserChatRooms() : Promise.resolve([]),
    ]);
            ...
            {activeTab === "admins" ? (
                            ...
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-zinc-800">
                                    {admins.length === 0 ? (
                                        ...
                                        admins.map((admin) => {
                                            const clubMemberships = (admin.club_memberships ?? []) as Array<{
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-3 rounded-2xl border border-zinc-800 bg-zinc-950 px-5 py-4">
                        ...
                        <div>
                            <h2 className="text-lg font-semibold text-white">채팅 모니터링</h2>
                            <p className="text-sm text-zinc-400">
                                채팅 내용은 보지 않고, 누가 누구와 채팅 중인지와 미읽음 대상 연락처만 확인합니다.
                            </p>
                        </div>
                    </div>

                    <SuperuserChatTab locale={locale} rooms={chatRooms} />
                </div>
            )}
        </div>
    );
